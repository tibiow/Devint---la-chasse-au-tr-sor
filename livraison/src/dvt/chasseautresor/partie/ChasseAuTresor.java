...
public class ChasseAuTresor extends Jeu {
    ...
    public void enterAction() {
        ...
        if (card != null) {
            if(!cardView.getArbitre().getPlayer1().isaJeter())
                updateStatut(cardView.getArbitre().getPlayer1(), currentCard);
            else
                cardView.getArbitre().getPlayer1().setaJeter(false);

            if(!cardView.getArbitre().getPlayer2().isaJeter())
                updateStatut(cardView.getArbitre().getPlayer2(), card);
            else
                cardView.getArbitre().getPlayer2().setaJeter(false);
        }
